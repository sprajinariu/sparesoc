...
- Masters/slaves: parameterized — 3+N masters, 7+N slaves (N = number of enabled accelerators; +1 for crypto)
  - Default (sim): 7 masters, 11 slaves (all 4 accelerators enabled + crypto)
  - FPGA (Basys 3): 3 masters, 7 slaves (all accelerators disabled, crypto always present)
...
- Accelerators: all disabled (`EnableReLU/VMAC/SgDma/Softmax = 0`) → 3 masters, 7 slaves
- AXI latency: `CUT_ALL_PORTS`
